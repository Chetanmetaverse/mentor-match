...
const createpay = async (req, res) => {
  try {
    await linkedIn.query(queryOptions).then((response) => {
      res.status(201).json({
        response,
        success: true,
...
const editExp = async (req, res) => {
  try {
    const { user, cost, desc, expanseDate, tittle, detail, expenses_id } =
      req.body;

    const editexp = await expanses.findByIdAndUpdate(expenses_id, {
      user,
      cost,
      desc,
      expanseDate,
...
module.exports = { createExp, deleteExp, getAll, editExp, createpay };
